...
fn expiration_slack_is_upheld() {
    ...
    // Create at least one block so that we can test an already expired tx as well.
    let (number, _) = uut.select_block();
    let header = BlockHeader::mock(number, None, None, &[], Word::default());
    uut.commit_block(header);

    let from = uut.chain_tip.parent().expect("we have at least one block");
    let limit = uut.chain_tip + uut.config.expiration_slack;

    for i in from.as_u32()..=limit.as_u32() {
    ...
    // Test that it passes if its beyond the limit.
    let tx = MockProvenTxBuilder::with_account_index(0)
