...
pub(crate) async fn run(
    ...
    assert_eq!(
        protocol_config.to_commitment(),
        ref_block_header.protocol_config_commitment(),
        "the target chain changed protocol configuration after genesis; regenerate with a client that supports the active configuration",
    );
