...
use crate::server::rate_limiter::RateLimiterConfig;
        ...
        Command::Start {
            ...
            let server_future = async {
                let server_handle = tokio::spawn(async move {
                    server.serve(endpoint, rate_limiter).await.context("server failed")
                });
...
mod test {
    ...
    use crate::{
        API_KEY_PREFIX, Cli, run_faucet_command, server::rate_limiter::RateLimiterConfig,
                ...
                run_faucet_command(Cli {
                    command: crate::Command::Start {
                        ...
                        rate_limiter: RateLimiterConfig {
                            ip_burst_size: 8,
                            ip_per_second: 1,
                            account_burst_size: 1,
                            account_per_second: 10,
                            api_key_burst_size: 3,
                            api_key_per_second: 10,
                        },
                    },
                })
                .await
