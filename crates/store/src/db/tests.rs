...
fn test_cleanup_old_account_vault_assets() {
    use crate::db::queries::{
        MAX_HISTORICAL_ENTRIES_PER_ACCOUNT,
        cleanup_old_account_vault_assets,
    };
    use crate::db::schema::account_vault_assets::dsl;
    ...
    let vault_key = num_to_word(100);
    let asset_base = Asset::Fungible(FungibleAsset::new(account_id, 1000).unwrap());
    ...
    // We should have deleted entries beyond MAX_HISTORICAL_ENTRIES_PER_ACCOUNT
    // The latest entry (is_latest_update=true) is always kept
    // Plus up to MAX_HISTORICAL_ENTRIES_PER_ACCOUNT non-latest entries
    let expected_remaining = MAX_HISTORICAL_ENTRIES_PER_ACCOUNT + 1; // +1 for the latest
    ...
    let latest_count = dsl::account_vault_assets
        .filter(dsl::account_id.eq(account_id.to_bytes()))
        .filter(dsl::is_latest_update.eq(true))
        .count()
        .get_result::<i64>(&mut conn)
        .unwrap();
...
fn test_cleanup_old_account_storage_map_values() {
    ...
    use crate::db::schema::account_storage_map_values::dsl;
    ...
    // Insert 70 storage map value entries for the same (slot, key) combination
...
fn test_cleanup_preserves_latest_state() {
    use crate::db::queries::cleanup_old_account_vault_assets;
    use crate::db::schema::account_vault_assets::dsl;
...
fn test_cleanup_all_accounts() {
    use crate::db::queries::cleanup_all_accounts;
    use crate::db::schema::account_vault_assets::dsl;
    ...
    // Use the same faucet for both accounts since fungible assets need a faucet account ID
    let vault_key = num_to_word(100);
    ...
    // Each account should have MAX_HISTORICAL_ENTRIES_PER_ACCOUNT + 1 entries remaining using
    // Diesel API
    ...
    assert_eq!(count1 as usize, crate::db::queries::MAX_HISTORICAL_ENTRIES_PER_ACCOUNT + 1);
    assert_eq!(count2 as usize, crate::db::queries::MAX_HISTORICAL_ENTRIES_PER_ACCOUNT + 1);
}
