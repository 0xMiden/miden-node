...
use crate::{
    EMPTY_WORD, InnerNode, Map, NodeIndex, SmtLeaf, SmtStorage, StorageError, StorageUpdateParts,
    StorageUpdates, Subtree, SubtreeUpdate, Word,
};
...
impl SmtStorage for RocksDbStorage {
    ...
    fn get_subtrees(&self, indices: &[NodeIndex]) -> Result<Vec<Option<Subtree>>, StorageError> {
        let keys: Vec<(NodeIndex, KeyBytes)> = indices
            .iter()
            .map(|&idx| (idx, Self::subtree_db_key(idx)))
            .collect();

        let key_refs: Vec<(&rocksdb::ColumnFamily, &[u8])> = keys
            .iter()
            .map(|(idx, k)| (self.subtree_cf(*idx), k.as_slice()))
            .collect();
