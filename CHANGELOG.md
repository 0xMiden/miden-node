...
- Block producer now detects if it is desync'd from the store's chain tip and aborts ([#1520](https://github.com/0xMiden/miden-node/pull/1520)).
  - This could occur in rare situations where the block is submitted to the store, and the request is only cancelled from the block-producer's perspective.
  - For example, due to a gRPC timeout.
