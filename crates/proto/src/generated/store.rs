...
pub struct NetworkUpdates {
    ...
    /// Complete set of network account updates in the inclusive range formed by
    /// the request's starting block and `end_inclusive`.
    ///
    /// The data is encoded using \[winter_utils::Serializable\] implementation for
    /// \[miden_objects::account::delta::AccountUpdateDetails\].
    ///
    /// Note that the type allows for `Private` but this isn't applicable to network
