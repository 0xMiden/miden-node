...
impl StoreClient {
    ...
    pub async fn latest_header(&self) -> Result<BlockHeader, StoreError> {
        let response = self
            ...
            .get_block_header_by_number(tonic::Request::new(
                shared_proto::GetBlockHeaderByNumber::default(),
            ))
            .await?
            .into_inner()
            .block_header
    ...
    pub async fn get_current_blockchain_data(
        ...
        let request = tonic::Request::new(store_proto::GetCurrentBlockchainData {
    ...
    pub async fn get_unconsumed_network_notes(&self) -> Result<Vec<NetworkNote>, StoreError> {
        ...
        loop {
            let req = store_proto::GetUnconsumedNetworkNotes { page_token, page_size: 128 };
