...
///
/// The transport layer connection is established lazily when the first block is proven.
#[derive(Clone)]
...
impl RemoteBlockProver {
    ...
    async fn connect(&self) -> Result<(), RemoteProverClientError> {
        ...
        let new_client = {
            ...
            proto::api_client::ApiClient::new(web_client)
        };
        ...
        let new_client = {
            ...
            let channel = endpoint
                ...
                .await
                .map_err(|err| RemoteProverClientError::ConnectionFailed(err.into()))?;
            proto::api_client::ApiClient::new(channel)
        };
